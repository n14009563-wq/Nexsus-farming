...
## v0.1.3 -- fix a malformed XML comment that broke the POM entirely

v0.1.2's own fix introduced a new bug: the explanatory comment added to `pom.xml` used a
double-hyphen ( `--` ) as a prose separator in two places, which XML comments don't allow
anywhere in their body (only as the closing `-->`). Maven couldn't even parse the POM as a
result:

```
[FATAL] Non-parseable POM /workspaces/Nexsus-farming/pom.xml: in comment after two dashes (--)
next character must be > not \n
```

This one never should have shipped -- a comment is exactly the kind of change that's easy to
write without noticing it isn't valid XML, and I hadn't actually parsed the file before packaging
it. Fixed by rewording the comment to drop every `--`, and this time verified with an actual XML
parse (`xml.dom.minidom`) before packaging, not just a visual read-through, so a malformed-XML
regression like this gets caught before delivery instead of at your next `mvn clean package`.

No other changes -- same `maven.test.skip=true`, same `-release 21`, same everything else from
v0.1.2.
